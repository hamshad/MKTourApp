...
class GooglePayHandler(private val activity: Activity) : MethodChannel.MethodCallHandler {
    ...
    private var isTestEnv: Boolean = true
    ...
    private fun getPaymentsClient(testEnv: Boolean): PaymentsClient {
        if (paymentsClient == null || isTestEnv != testEnv) {
            isTestEnv = testEnv
            val environment = if (testEnv) {
                WalletConstants.ENVIRONMENT_TEST
            } else {
                WalletConstants.ENVIRONMENT_PRODUCTION
            }
            Log.d(TAG, "Creating PaymentsClient with environment: ${if (testEnv) "TEST" else "PRODUCTION"}")
    ...
    private fun checkGooglePayAvailable(call: MethodCall, result: MethodChannel.Result) {
        val testEnv = call.argument<Boolean>("testEnv") ?: true
    ...
    fun handleActivityResult(requestCode: Int, resultCode: Int, data: Intent?): Boolean {
        ...
        when (resultCode) {
            Activity.RESULT_OK -> {
                data?.let {
                    ...
                    if (paymentInfo != null) {
                        ...
                        pendingResult?.success(mapOf(
                            "success" to false,
                            "error" to "Payment data was null"
                        ))
                    }
                } ?: run {
                    pendingResult?.success(mapOf(
                        "success" to false,
                        "error" to "No data returned from Google Pay"
                    ))
                }
            }
            ...
            AutoResolveHelper.RESULT_ERROR -> {
                ...
                pendingResult?.success(mapOf(
                    "success" to false,
                    "error" to "Google Pay error: ${status?.statusMessage ?: "Unknown error"} (${status?.statusCode})"
                ))
            }
        }
        pendingResult = null
    ...
    private fun getCardPaymentMethod(stripePublishableKey: String): JSONObject {
        ...
        base.put("tokenizationSpecification", JSONObject().apply {
    ...
    private fun getPaymentDataRequest(
        ...
        return JSONObject().apply {
            ...
            put("apiVersionMinor", 0)
            put("allowedPaymentMethods", JSONArray().apply {
                put(getCardPaymentMethod(stripePublishableKey))
            })
